...
def f_nn(params):
    ...
    model = Sequential()
    model.add(Conv2D(32, (3, 3), name="convolution2d_1", activation="relu", padding="same", kernel_initializer="orthogonal",
                     use_bias=True, input_shape=(52, 32, 1)))

    model.add(Conv2D(32, (3, 3), name="convolution2d_2", activation="relu", padding="same", kernel_initializer="orthogonal",
           use_bias=True))

    model.add(MaxPooling2D(pool_size=(2, 2), strides=(2, 2), name='maxpooling2d_1'))

    model.add(Conv2D(16, (3, 3), name="convolution2d_3", activation="relu", padding="same", kernel_initializer="orthogonal",
           use_bias=True))
    model.add(Conv2D(16, (3, 3), name="convolution2d_4", activation="relu", padding="same", kernel_initializer="orthogonal",
           use_bias=True))

    model.add(MaxPooling2D(pool_size=(2, 2), strides=(2, 2), name='maxpooling2d_2'))

    model.add(Conv2D(16, (3, 3), name="convolution2d_5", activation="relu", padding="same", kernel_initializer="orthogonal",
           use_bias=True))
    model.add(Conv2D(16, (3, 3), name="convolution2d_6", activation="relu", padding="same", kernel_initializer="orthogonal",
           use_bias=True))

    model.add(Flatten(name='flatten_1'))
    model.add(BatchNormalization())
    model.add(Dense(units=params['units1'], name="dense_1", activation="relu", use_bias=True))
    ...
    writer.writerow([-acc, params, ITERATION])
...
if __name__ == "__main__":
    ...
    writer.writerow(['accuracy', 'params', 'iteration'])
    of_connection.close()
